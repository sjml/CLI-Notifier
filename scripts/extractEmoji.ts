...
export function writeAppIconSet(emoData: Uint8Array, outputPath: string) {
  ...
  SIZES.forEach((dim) => {
    let cmd = new Deno.Command("sips", {
      ...
      args: [
        "-Z", String(dim),
        "--out", `${outputPath}/icon_${dim}x${dim}.png`,
        baseFile,
      ],
    });
    cmd.outputSync();
    ...
    cmd = new Deno.Command("sips", {
      ...
      args: [
        "-Z", String(doubleSize),
        "--out", `${outputPath}/icon_${dim}x${dim}@2x.png`,
        baseFile,
      ],
    });
    cmd.outputSync();
  ...
  Deno.writeTextFileSync(
    `${outputPath}/Contents.json`,
    JSON.stringify(content_datums, null, 2),
  );
}
...
function main() {
  try {
    ...
    if (
      flags.flags.file === undefined && flags.flags.appiconset === undefined
    ) {
      console.error(
        'Error: Either "--file" or "--appiconset" must be specified.',
      );
      Deno.exit(1);
    }
