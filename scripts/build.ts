...
if (!fs.existsSync(BASE_APP)) {
  ...
  Deno.renameSync(
    path.join(archivePath, "Products", "Applications", "Teller.app"),
    BASE_APP,
  );
...
fs.copySync(
  path.join(ROOT_DIR, "Teller", "Assets.xcassets"),
...
for (const bd of jsonData.builds) {
  if (bd.label.length > 8) {
    console.error(
      `Error: “${bd.label}” is too long of a label; must be ≤ 8 characters.`,
    );
    Deno.exit(1);
  }
  if (/[^A-Za-z0-9\-\.]/g.test(bd.label)) {
    console.error(
      `Error: “${bd.label}” is invalid; can only contain A-Z, a-z, 0-9, hyphens, and periods.`,
    );
    Deno.exit(1);
  }
  ...
  Deno.removeSync(path.join(TMP_DIR, "Assets.xcassets", "AppIcon.appiconset"), {
    recursive: true,
  });
  fs.copySync(
    appIconSetPath,
    path.join(TMP_DIR, "Assets.xcassets", "AppIcon.appiconset"),
  );
  ...
  fs.copySync(
    path.join(TMP_DIR, "assets", bd.label),
    path.join(customApp, "Contents", "Resources"),
    { overwrite: true },
  );
  ...
  try {
    Deno.removeSync(path.join(BUILD_DIR, `Teller-${bd.label}.app`), {
      recursive: true,
    });
  } catch {
    //no-op
